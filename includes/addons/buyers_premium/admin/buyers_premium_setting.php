...
if ( isset( $_POST['uwa_buyers_premium_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['uwa_buyers_premium_nonce'] ) ), 'uwa_buyers_premium_save' ) && isset( $_POST['uwa-buyers_premium-submit'] ) ) {
	...
	if ( ! current_user_can( 'manage_options' ) ) {
		...
		if (isset($_POST['uwa_buyers_premium_rate'])) {
			update_option( 'uwa_buyers_premium_rate', (float) sanitize_text_field( wp_unslash( $_POST['uwa_buyers_premium_rate'] ) ) ); // save float value
		}

		if (isset($_POST['uwa_buyers_min_premium'])) {
			update_option( 'uwa_buyers_min_premium', (float) sanitize_text_field( wp_unslash( $_POST['uwa_buyers_min_premium'] ) ) );
		}

		if (isset($_POST['uwa_buyers_max_premium'])) {
			update_option( 'uwa_buyers_max_premium', (float) sanitize_text_field( wp_unslash( $_POST['uwa_buyers_max_premium'] ) ) );
		}
		...
		if (isset($_POST['uwa_buyers_fee_title'])){
			update_option( 'uwa_buyers_fee_title', sanitize_text_field( wp_unslash( $_POST['uwa_buyers_fee_title'] ) ) );
		}
	...
	<form id="uwa_buyers_premium_form" class="auction_settings_section_style" action=""
		...
		<table class="form-table">
			...
			<tr valign="top">
				<th scope="row">
					<label for="uwa_buyers_premium_type">
						<?php esc_html_e( "Buyer's Premium Type", "woo_ua" ); ?>
					</label>
				</th>
				<td>
					...
					<?php esc_html_e( 'Flat Rate', 'woo_ua' ); ?>
					<span style="margin-right:20px;"></span>

					<input type="radio" <?php echo ( $uwa_buyers_premium_type === 'percentage' )
						? 'checked' : ''; ?> name="uwa_buyers_premium_type"
						id="uwa_stripe_charge_type_partially" value="percentage" >
					<?php esc_html_e( 'Percentage', 'woo_ua' ); ?>
				</td>
			</tr>

			<tr valign="top">
				<th scope="row">
					<label for="uwa_buyers_premium_rate"><?php esc_html_e( "Fee Amount", "woo_ua" ); ?></label>
				</th>

				<td>
					<input name="uwa_buyers_premium_rate" type="text"
						id="uwa_buyers_premium_rate"  value="<?php echo esc_attr( $uwa_buyers_premium_rate ); ?>"  size="14" />
					<?php printf( esc_html__( "Based on your selection above this field is Amount for Flat Rate or Percentage.", 'woo_ua' ), esc_html( get_woocommerce_currency_symbol() ) ); ?>
				</td>
			</tr>
			...
			<tr valign="top" class="uwa_min_max">
				<th scope="row">
					<label class="lbl_min_max" for="uwa_buyers_min_premium"><?php esc_html_e( "Minimum Premium Amount", "woo_ua" ); ?></label>
				</th>
				<td><div class="dv_min_max">
					<?php printf( "%s", esc_html( get_woocommerce_currency_symbol() ) ); ?>
					<input name="uwa_buyers_min_premium" type="number"
						id="uwa_buyers_min_premium"  min="1"
						title="used for percentage only"
						disabled   value="<?php echo esc_attr( $uwa_buyers_min_premium ); ?>"  size="14" />
					<?php printf( esc_html__( "This amount is minimum buyer's premium amount in unit of currency that will be applicable. If the amount calculated in percentage is below this minimum amount then this amount will be charged.", 'woo_ua' ), esc_html( get_woocommerce_currency_symbol() ) ); ?>
				</div>
				</td>
			</tr>
			...
			<tr valign="top" class="uwa_min_max">
				<th scope="row">
					<label class="lbl_min_max" for="uwa_buyers_max_premium"><?php esc_html_e( "Maximum Premium Amount", "woo_ua" ); ?></label>
				</th>
				<td><div class="dv_min_max">
					<?php printf( "%s", esc_html( get_woocommerce_currency_symbol() ) ); ?>
					<input name="uwa_buyers_max_premium" type="number"
						...
						title="used for percentage only"
						disabled value="<?php echo esc_attr( $uwa_buyers_max_premium ); ?>"  size="14" />
					<?php printf( esc_html__( "This amount is maximum buyer's premium amount in unit of currency that will be applicable. If the amount calculated in percentage is above this maximum amount then this amount will be charged.", 'woo_ua' ), esc_html( get_woocommerce_currency_symbol() ) ); ?>
				</div>
				</td>
			</tr>
			...
			<tr>
				<th scope="row"></th>
				<td>
					<input <?php echo esc_attr( $is_hide_tab ); ?> value="1"
						name="uwa_buyers_premium_tab_hide" type="checkbox">
					<?php esc_html_e( "Hide Buyer's Premium tab on auction detail page.", 'woo_ua' ); ?>
				</td>
			</tr>
			...
			<tr>
				<th scope="row"></th>
				<td>
					<input <?php echo esc_attr( $uwa_aft_btn ); ?> value="1"
						name="uwa_buyers_premium_aft_btn" type="checkbox">
					<?php esc_html_e( "Display Buyer's Premium after bid button on auction detail page.", 'woo_ua' ); ?>

				</td>
			</tr>

			<tr class="uwa_heading">
				<th colspan="2"><?php esc_html_e( "Buyer's premium fee title on checkout page", "woo_ua" ); ?></th>
			</tr>
			...
			<tr>
				...
				<td>
					<a href="" class="uwa_fields_tooltip" onclick="return false">
						<strong>?</strong>
						<span><?php esc_html_e( "e.g. SKU - #39057 then <br> <br> FEE TITLE like - Buyer's premium fee for #39057", "woo_ua" ); ?>
						</span>
					</a>
					<input type="radio" <?php echo ( $uwa_buyers_fee_title === "fee_sku" ) ?
						'checked' : ''; ?> name="uwa_buyers_fee_title"
						id="uwa_buyers_fee_sku" value="fee_sku">
					<?php esc_html_e( "Display SKU", "woo_ua" ); ?>
			...
			<tr>
				...
				<td>
					<a href="" class="uwa_fields_tooltip" onclick="return false">
						<strong>?</strong>
						<span><?php esc_html_e( "e.g. PRODUCT TITLE - Abstract Painting then <br> <br> FEE TITLE like - Buyer's premium fee for Abstract Painting", "woo_ua" ); ?>
						</span>
					</a>
					<input type="radio" <?php echo ( $uwa_buyers_fee_title === "fee_product_title" )
						? 'checked' : ''; ?> name="uwa_buyers_fee_title"
						id="uwa_buyers_fee_product_title" value="fee_product_title" >
					<?php esc_html_e( "Display product title", "woo_ua" ); ?>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<a href="" class="uwa_fields_tooltip" onclick="return false">
						<strong>?</strong>
						<span><?php esc_html_e( "e.g. PRODUCT TITLE - Vikings Valhalla Olaf Haraldsson Jóhannes Haukur Jóhannesson Screen Used Flag Set then <br> <br> FEE TITLE like - Buyer's premium fee for Vikings Valhalla Olaf Haraldsson Jóhannes Haukur...", "woo_ua" ); ?>
						</span>
					</a>
					<input type="radio" <?php echo ( $uwa_buyers_fee_title === "fee_trim_product_name" )
						? 'checked' : ''; ?> name="uwa_buyers_fee_title"
						id="uwa_buyers_fee_trim_product_name" value="fee_trim_product_name" >
					<?php esc_html_e( "Trim product title if longer than 60 characters", "woo_ua" ); ?>
				</td>
			</tr>

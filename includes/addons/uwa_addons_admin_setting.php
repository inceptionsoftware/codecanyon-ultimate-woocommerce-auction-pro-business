				...
				<?php
					foreach( $uwa_addons_setting_tabs as $key => $addontab ){
						if(in_array($addontab['slug'], $uwa_enabled_addons_list)) { ?>
							<li>
								<a href="?page=uwa_general_setting&setting_section=uwa_addons_setting&uwa_addons_tab=<?php echo esc_attr( $addontab['slug'] ); ?>"
								class="<?php echo esc_attr( $active_addon_tab === $addontab['slug'] ? 'current' : '' ); ?>"><?php echo esc_html( $addontab['name'] ); ?></a>
							</li>
							<?php

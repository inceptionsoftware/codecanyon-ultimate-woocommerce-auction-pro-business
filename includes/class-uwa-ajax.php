...
class UWA_AJAX {
	...
	public static function define_uwa_ajax() {
		if ( ! empty( $_GET['uwa-ajax'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public static function do_uwa_ajax() {
		...
		if ( ! empty( $_GET['uwa-ajax'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			do_action( 'uwa_ajax_' . sanitize_text_field( wp_unslash( $_GET['uwa-ajax'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			wp_die();
		}
	}
